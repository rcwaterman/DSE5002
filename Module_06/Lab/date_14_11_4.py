...
class Date():
    ...
    def _validate_year(self, year) -> None:
        ...
        # Per the article, below, the first time a leap year was instituted 
        # was 46 B.C.
    ...
    def make_date(self, year:int, month:Union[int, str], day:int) -> str:
        ...
        NOTE: 
        The validation order is critical, and must occur in year, month, day 
        order. Both the year and month must be valid before checking for leap 
        year. If it is a leap year, February will have an additional day, which 
        will be validated against the self.MONTH_MAPPING dictionary.
        ...
        functions are used to implement error checking for the respective 
        inputs. Also, this allows for accurate date checking with leap years 
        accounted for.
        """
    ...
    def print_date(self) -> str:
        ...
        """
        if self.bc:
            print(f"{self.year}-{self.month}-{self.day} BC")
        else:
            print(f"{self.year}-{self.month}-{self.day}")

def is_after(date1:Date, date2:Date) -> bool:
    """
    Determine if the first date comes after the second.
    """
    if date1.date_to_int() > date2.date_to_int():
        return True
    else:
        return False
